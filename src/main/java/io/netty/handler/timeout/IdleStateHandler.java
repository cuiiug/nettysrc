...
public class IdleStateHandler extends ChannelDuplexHandler {
	...
	// Not create a new ChannelFutureListener per write operation to reduce GC
	// pressure.
	 ...
	 *
	 * @param observeOutput
	 *            whether or not the consumption of {@code bytes} should be taken
	 *            into consideration when assessing write idleness. The default is
	 *            {@code false}.
	...
	public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
		// Allow writing with void promise if handler is only configured for read
		// timeout events.
	...
	/**
	 * Is called when an {@link IdleStateEvent} should be fired. This implementation
	 * calls {@link ChannelHandlerContext#fireUserEventTriggered(Object)}.
	 */
	...
	/**
	 * Returns {@code true} if and only if the {@link IdleStateHandler} was
	 * constructed with {@link #observeOutput} enabled and there has been an
	 * observed change in the {@link ChannelOutboundBuffer} between two consecutive
	...
	private boolean hasOutputChanged(ChannelHandlerContext ctx, boolean first) {
		if (observeOutput) {
			...
			// appear to complete. It indicates "change" on message level and we simply
			// assume
			// that there's change happening on byte level. If the user doesn't observe
			// channel
			// writability events then they'll eventually OOME and there's clearly a
			// different
